...
?> ScreenShare exposes a STUN and TURN server. You don't have to configure this separately.
...
When STUN is used, only the connection enstablishment will be done through ScreenShare. The actual video stream will be
directly sent to the other peer and doesn't go through ScreenShare.
...
ScreenShare will require user authentication to use the TURN server. This can be configured see [Configuration](config.md).
